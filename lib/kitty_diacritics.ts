...
/**
 * Reverse lookup: codepoint → integer index. Built once at module load.
 * Returns -1 for codepoints that aren't valid kitty diacritics.
 */
const DIACRITIC_INDEX = new Map<number, number>(
  ROWCOLUMN_DIACRITICS.map((cp, i) => [cp, i]),
);

export function diacriticToInt(cp: number): number {
  return DIACRITIC_INDEX.get(cp) ?? -1;
}
