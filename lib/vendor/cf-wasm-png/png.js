...
function encode(image, width, height, palette, trns, color, depth, compression, filter) {
  ...
  const ret = wasm.encode(
    ...
    ptr2,
    len2,
    isLikeNone(color) ? 16777215 : color,
    isLikeNone(depth) ? 16777215 : depth,
    isLikeNone(compression) ? 16777215 : compression,
    isLikeNone(filter) ? 16777215 : filter
  );
...
const DecodeResultFinalization =
  ...
  if (typeof Response === 'function' && module instanceof Response) {
    if (typeof WebAssembly.instantiateStreaming === 'function')
      try {
        ...
        if (
          module.ok &&
          expectedResponseType(module.type) &&
